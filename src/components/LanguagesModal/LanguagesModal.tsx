...
export const LanguagesModal = ({ current, close, forLang, ...props }: LanguagesModalProps): JSX.Element => {
  const { register, setFocus, handleSubmit, watch, resetField } = useForm<ISearch>({ defaultValues: { search: '' } });
  ...
  return createPortal(
    <div className={styles.modal}>
      <form className={styles.form} onSubmit={handleSubmit(onSubmit)} {...props}>
        ...
        <button type='button' onClick={() => resetField('search')} className={styles.clearBtn}>
          <Icon type='clear' />
        </button>
      </form>
